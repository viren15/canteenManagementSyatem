	...
	<span style="font-size:30px;cursor:pointer" onclick="openNav()">&#9776;</span>
        <div style="width:400px; margin:50px auto; border:solid #03021c 1px">
                ...
                <form action="#" method="post" style="padding:20px 50px" label="Log In">
                        <button type="submit">Fetch Food</button>
                        <br />
        ...
        <?php
                if($_SERVER["REQUEST_METHOD"] == "POST"){
                        ...
                        while($row = $result->fetch_assoc()) {
                                ...
                                <hr />
                                <p align='left'><b>food Name:</b> {$row["food_name"]}</p>
                                <p align='left'><b>Food Id: </b>{$row["food_id"]}</p >
                                <p align='left'><b>Food desc: </b>{$row["food_desc"]}</p >
                                <p align='left'><b>Food Type: </b>{$row["food_type"]}</p >
                                <p align='left'><b>Food Price: </b>{$row["food_price"]}$</p>
                                <p align='left'><b>Food Prep Time: </b>{$row["food_preptime"]} min</p>
                                <p align='left'><b>Availability: {$row["available"]}</b></p>
                                <hr />
                                </div>";
                        }
                }
